...
在访问 CLIC 前反向转换。公共 `include/irq.h` 统一使用
`BL616CL_IRQ_NUM_*` 表示 NuttX 编号，LHAL 的 `BL616CL_IRQ_*` 表示 raw CLIC
索引，两套名称不得混用。私有转换接口放在 `bl616cl_irq_internal.h`，避免与
LHAL 的 `bl616cl_irq.h` 同名遮蔽。

当前 LHAL IRQ 表及设备表是槽位语义的依据；std 枚举保留了部分旧名称。例如
槽位 20 的当前设备语义是 `TOUCH_V2`，不能把它表述成 `IRRX` 的兼容 alias。
`bl616cl_irq.c` 编译期逐项检查全部 74 个公共编号与 LHAL raw 编号的加 16
关系，覆盖 7 个 RV32 核心异步中断及 67 个芯片槽位（含保留槽）；std 枚举仅
校验条目数量和编号范围。IRQ 的加 16 是真实编号适配，因此它不同于地址或
位域的纯重命名。
