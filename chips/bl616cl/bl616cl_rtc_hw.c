...
{
  (void)HBN_Set_RTC_CLK_Sel(HBN_RTC_CLK_F32K);

#ifdef CONFIG_BL616CL_RTC_CLOCK_DIG32K
  (void)GLB_Set_DIG_CLK_Sel(GLB_DIG_CLK_XCLK);
  (void)GLB_Set_DIG_32K_CLK(ENABLE, DISABLE, BL616CL_RTC_DIG32K_DIV);
  (void)HBN_32K_Sel(HBN_32K_DIG);
#else
  (void)HBN_Keep_On_RC32K();
  (void)HBN_32K_Sel(HBN_32K_RC);
#endif

  (void)HBN_Enable_RTC_Counter();
}

void bl616cl_rtc_hw_counter(uint32_t *low, uint32_t *high)
{
  (void)HBN_Get_RTC_Timer_Val(low, high);
}

#ifdef CONFIG_BL616CL_RTC_ALARM
void bl616cl_rtc_hw_set_alarm(uint64_t counter)
{
  (void)HBN_Set_RTC_Timer(HBN_RTC_INT_DELAY_0T, (uint32_t)counter,
                          (uint32_t)(counter >> 32),
...
void bl616cl_rtc_hw_clear_alarm(void)
{
  ...
  (void)HBN_Clear_IRQ(HBN_INT_RTC);
}
